  ...
  return (
    <div className={`card ${styles.listProductCard}`}>
        <div className="row g-0">
          ...
          <Link href={`/product/${product.product_id}`} passHref className="text-decoration-none">
            <Image
              src={imageUrl}
          ...
          <div className="col-lg-8 col-6 mt-2">
            <div className={`d-flex flex-column justify-content-between h-100 ${styles.listProductCardText}`}>
              <div>
                <div className="d-flex justify-content-between align-items-center">
                  <h5 className={styles.listProductCardTitle}>{product.product_name.slice(0, 20)}</h5>
                  <FavFcon
                    id={product.product_id}
                    style={`${styles.listHeartIcon} d-flex justify-content-end`}
                  />
                </div>
                <p className={`${styles.listProductCardCategory}`}>{product.product_category}</p>
              </div>
              <div>
                <div className={`card-text d-flex justify-content-between mt-3 `}>
                  <p className={`text-primary-dark fw-bold ${styles.listCurrency}`}>
